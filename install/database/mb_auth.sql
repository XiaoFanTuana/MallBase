...
CREATE TABLE `mb_permission` (
  ...
  `redirect` varchar(255) DEFAULT NULL COMMENT '重定向路径' AFTER `component`,
  `affix_tab` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否固定标签页：0=不固定 1=固定（不可关闭）' AFTER `is_show`,
  `no_basic_layout` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否需要基础布局：0=需要 1=不需要' AFTER `affix_tab`,
  `sort` int(11) NOT NULL DEFAULT 0 COMMENT '排序',
  `status` tinyint(1) NOT NULL DEFAULT 1 COMMENT '状态：0=禁用 1=启用',
