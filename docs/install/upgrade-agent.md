...
本仓库中的 `upgrade/bin/mallbase-agent-linux-amd64` 和 `mallbase-agent-linux-arm64` 是上游预编译、已 strip 的静态 Go 二进制，MallBase 仓库本身不包含对应 Agent 源码。`checksums.sha256` 只能证明部署文件与仓库提交一致，不能替代源码审计、可复现构建或发布签名验证。

私有 fork 默认使用 `PLATFORM_REPORT_DISABLED=true`，不会自动执行实例激活或低频心跳。只有在你已经独立确认 Agent 来源并接受它访问平台、修改宿主机代码树、执行数据库备份与迁移的权限边界后，才应安装下面的 systemd 单元并把该开关改为 `false`。Docker Hub 不可变镜像部署不使用宿主机在线升级 Agent，应通过镜像拉取与容器重建更新。
