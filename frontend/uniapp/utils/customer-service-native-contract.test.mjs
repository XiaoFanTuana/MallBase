...
function createUnauthorizedHarness(currentRoute = 'pages/profile/index') {
  const start = requestSource.indexOf('const UNAUTHORIZED_REDIRECT_DEDUP_MS')
  const end = requestSource.indexOf('function refreshAccessToken')
  assert.notEqual(start, -1, 'request unauthorized redirect constants must exist')
  assert.ok(end > start, 'request unauthorized redirect handler must exist')
  ...
  const clearAuthSession = () => {
    removedKeys.push('mb_access_token', 'mb_refresh_token')
  }
  ...
  return {
    handleUnauthorized: create(uni, getCurrentPages, clearAuthSession, notifyAuthCleared),
    authClearedEvents,
    navigateCalls,
    removedKeys,
  }
...
test('voice recording uses platform adapters and external upload accepts H5 blobs', () => {
  ...
  assert.match(recorderSource, /nativeRecorderSession !== null/)
