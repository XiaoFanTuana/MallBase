...
>({ delete: deleteFreightTemplateApi, list: getFreightTemplateListApi }, { immediateLoad: false });
...
const handleStatusChange = async (record: FreightTemplateApi.TemplateItem, checked: boolean) => {
...
const columns = [
  ...
  { title: '模板名称', dataIndex: 'name', width: 180 },
  {
    ...
    width: 100,
    customRender: ({ record }: { record: FreightTemplateApi.TemplateItem }) => h(Tag, { color: 'blue' }, () => record.charge_type === 'weight' ? '按重' : '按件'),
  },
  { title: '规则数', dataIndex: 'rule_count', width: 90 },
  {
    ...
    width: 100,
    customRender: ({ record }: { record: FreightTemplateApi.TemplateItem }) => h(Tag, { color: (record.invalid_rule_count || 0) > 0 ? 'error' : 'success' }, () => String(record.invalid_rule_count || 0)),
  },
  {
    title: '状态',
    width: 90,
    customRender: ({ record }: { record: FreightTemplateApi.TemplateItem }) => h(Switch, {
  ...
  <div class="p-4">
    ...
    <a-form layout="inline" class="mb-4">
      <a-form-item label="模板名称">
        <a-input v-model:value="searchParams.name" placeholder="请输入模板名称" allow-clear />
      </a-form-item>
    ...
    <FreightTemplateModal v-model:visible="modalVisible" :edit-data="editingItem" @success="() => loadData(searchParams)" />
  </div>
</template>
