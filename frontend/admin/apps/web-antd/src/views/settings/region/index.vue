...
>({ delete: deleteRegionApi, list: getRegionListApi }, { immediateLoad: false });
...
const editingItem = ref<(RegionApi.RegionItem & { path?: RegionApi.RegionItem[] }) | null>(null);
...
const handleStatusChange = async (record: RegionApi.RegionItem, checked: boolean) => {
  await updateRegionStatusApi(record.id, checked ? 1 : 0);
  message.success('状态更新成功');
  await loadData(searchParams.value);
};

const levelMap: Record<number, string> = { 1: '省', 2: '市', 3: '区县', 4: '街道' };
const columns = [
  ...
  { title: '编码', dataIndex: 'code', width: 140 },
  {
    ...
    dataIndex: 'level',
    width: 90,
    customRender: ({ record }: { record: RegionApi.RegionItem }) => h(Tag, { color: 'blue' }, () => levelMap[record.level] || String(record.level)),
  },
  ...
  { title: '路径编码', dataIndex: 'path_codes', ellipsis: true },
  {
    title: '状态',
    width: 90,
    customRender: ({ record }: { record: RegionApi.RegionItem }) => h(Switch, {
      checked: record.status === 1,
      checkedChildren: '启用',
      unCheckedChildren: '禁用',
      onChange: (checked: any) => handleStatusChange(record, Boolean(checked)),
    }),
  },
  ...
  <div class="p-4">
    ...
    <a-form layout="inline" class="mb-4">
      <a-form-item label="关键词">
        <a-input v-model:value="searchParams.keyword" allow-clear placeholder="名称/编码" />
      </a-form-item>
      <a-form-item label="层级">
        <a-select v-model:value="searchParams.level" allow-clear style="width: 120px">
      ...
      <a-form-item label="状态">
        <a-select v-model:value="searchParams.status" allow-clear style="width: 120px">
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'action'">
          <a-space>
            ...
            <a-button type="link" danger size="small" @click="handleDelete(record, 'name')">删除</a-button>
          </a-space>
        </template>
      </template>
    </a-table>
    <RegionModal v-model:visible="modalVisible" :edit-data="editingItem" @success="() => loadData(searchParams)" />
  </div>
</template>
