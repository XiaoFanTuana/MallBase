...
CREATE TABLE `mb_sms_provider` (
  ...
  `driver` varchar(20) NOT NULL COMMENT '驱动类型：aliyun/tencent/mock',
...
CREATE TABLE `mb_sms_template` (
  ...
  `template_code` varchar(80) NOT NULL DEFAULT '' COMMENT '模板编码（阿里云返回的 SMS_xxx，本地新建未审核时为空）',
...
CREATE TABLE `mb_sms_scene_binding` (
  ...
  `sign_id` int(11) unsigned NOT NULL COMMENT '签名ID',
  `status` tinyint(1) NOT NULL DEFAULT 1 COMMENT '状态：0=禁用 1=启用',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
